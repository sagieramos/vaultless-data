-- ============================================================================
-- Migration: Update views and functions to join developer_subscriptions on application_id
-- Description: Now that developer_subscriptions has application_id, join directly
--              instead of going through developer_id.
-- ============================================================================
...
-- 2. Recreate mv_applications_with_usage joining on application_id
...
SELECT
    ...
    -- SUBSCRIPTION (resolved via application_id)
...
LEFT JOIN public.developer_subscriptions s ON (s.application_id = a.id AND s.is_active = true)
...
-- 3. Recreate indexes for mv_applications_with_usage
...
-- 4. Recreate mv_attached_apps_by_plan
...
-- 7. Update auth functions to join on application_id
...
-- 8. Update create_application to resolve subscription via application_id
...
BEGIN
    ...
    -- Get active subscription for this application
    SELECT ds.id INTO v_subscription_id
    FROM developer_subscriptions ds
    WHERE ds.application_id = v_app_id AND ds.is_active = TRUE
    LIMIT 1;
...
-- 9. Update update_application to resolve subscription via application_id
