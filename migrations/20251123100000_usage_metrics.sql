...
SELECT create_hypertable('application_usage_metrics', 'period_start', if_not_exists => TRUE);

-- Primary lookup indices for raw data
-- Optimizing for: "Show me usage for this app" or "Show me usage for this specific key"
CREATE INDEX idx_usage_application_lookup ON application_usage_metrics(application_id, period_start DESC);
CREATE INDEX idx_usage_developer_subscription_lookup ON application_usage_metrics(subscription_id, period_start DESC);
CREATE INDEX idx_usage_api_key_lookup ON application_usage_metrics(api_key_id, period_start DESC);
...
ON application_usage_metrics(api_key_id, application_id, subscription_id, period_start)
WHERE api_key_id IS NOT NULL;

-- App-level usage (publishable / system)
CREATE UNIQUE INDEX idx_usage_app_period
ON application_usage_metrics (application_id, subscription_id, period_start)
WHERE api_key_id IS NULL;

-- Compression configuration
-- Segmenting by subscription and application makes cross-app billing queries very fast
ALTER TABLE application_usage_metrics SET (
...
SELECT add_compression_policy('application_usage_metrics', INTERVAL '7 days');
SELECT add_retention_policy('application_usage_metrics', INTERVAL '90 days');
...
CREATE MATERIALIZED VIEW application_usage_metrics_daily
...
CREATE MATERIALIZED VIEW application_usage_metrics_keys_daily
WITH (timescaledb.continuous) AS
SELECT
    application_id,
    api_key_id,
...
SELECT add_continuous_aggregate_policy(
    'application_usage_metrics_daily',
...
SELECT add_continuous_aggregate_policy(
    'application_usage_metrics_keys_daily',
    start_offset => INTERVAL '3 days',
    end_offset   => INTERVAL '0 hours',
    schedule_interval => INTERVAL '1 hour'
);